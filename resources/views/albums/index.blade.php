...
<head>
    ...
    <style>
        ...
        *{ box-sizing:border-box; }
        ...
        .sidebar-brand{ display:flex; align-items:center; gap:.6rem; padding:1.1rem 1rem .9rem; font-weight:700; font-size:.9rem; color:#fff; border-bottom:1px solid var(--panel-border); letter-spacing:.01em; flex-shrink:0; }
        ...
        .sidebar-artist-item.active{ color:var(--accent); }
        ...
        .album-card-link:hover .album-card{ border-color:rgba(13,202,240,0.4); transform:translateY(-3px); box-shadow:0 8px 26px rgba(13,202,240,.1); }
        .album-thumb-wrap{ aspect-ratio:1/1; overflow:hidden; background:#0a0e18; flex-shrink:0; }
        ...
        .album-info .meta{ font-size:.68rem; color:rgba(255,255,255,.25); margin-top:.3rem; flex-shrink:0; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        <a href="{{ route('albums.index') }}" class="sidebar-item active">
            <i class="bi bi-grid-fill"></i> Albums
        </a>
        <a href="{{ route('albums.drafts') }}" class="sidebar-item">
            <i class="bi bi-pencil-square"></i> Rascunhos
            @php $dc = \App\Models\Album::where('status','draft')->count() @endphp
            @if($dc > 0)<span class="sidebar-badge">{{ $dc }}</span>@endif
        </a>
    ...
    <div class="main-content">
        ...
        @if($byArtist->isEmpty())
            <div class="text-center py-5 text-secondary">
                ...
                <p class="small opacity-50">Importe um album e clique em "Salvar tudo" para publicar.</p>
            </div>
        @else
